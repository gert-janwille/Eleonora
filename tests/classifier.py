...
json_file = open('../eleonora/data/models/big_XCEPTION/model_69.json', 'r')
loaded_model_json = json_file.read()
json_file.close()

model = model_from_json(loaded_model_json)
model.load_weights("../eleonora/data/models/big_XCEPTION/model_69.h5")
...
def detect_face(image):
	...
	for (x,y,w,h) in faces:
		...
		image= np.array(image).reshape((len(image), width, height, 1))
		return True,image
	return False,image
